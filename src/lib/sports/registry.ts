...
export const SPORTS: readonly SportDefinition[] = [
  ...
  },
  {
    ...
    stats: TENNIS_STATS,
    models: ['elo', 'rank', 'markov', 'stats', 'ml', 'ai', 'ensemble'],
    phase: 2,
...
export const MODEL_DESCRIPTIONS: Record<string, string> = {
  ...
  rank: 'Turns the gap between the two players’ published tour rankings into a win chance. The gap counts in multiples rather than places, so number one against number ten matters far more than number three hundred against number three hundred and ten.',
  markov: 'Works out how often each player wins a point on serve, then plays the match out point by point through games, tiebreaks and sets. That gives both the win chance and how likely each set score is, and it is why a small edge on serve becomes a large edge on the match.',
